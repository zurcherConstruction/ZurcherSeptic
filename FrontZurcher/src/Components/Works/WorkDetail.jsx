...
const WorkDetail = () => {
  ...
  const handleVerifyInvoice = async (invoiceId) => {
    try {
      const { data } = await api.patch(`/supplier-invoices/${invoiceId}/verify`);
      setLinkedInvoices(prev =>
        prev.map(inv => inv.idSupplierInvoice === invoiceId ? { ...inv, verified: data.verified } : inv)
      );
    } catch (err) {
      console.error('Error al verificar invoice:', err);
    }
  };
                      ...
                      >
                        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                          ...
                          <div className="flex flex-col gap-2">
                            ...
                            <button
                              onClick={() => handleVerifyInvoice(invoice.idSupplierInvoice)}
                              className={`px-4 py-2 rounded text-sm font-medium transition-colors flex items-center justify-center gap-2 ${
                                invoice.verified
                            ...
                            >
                              ...
                              {invoice.verified ? 'Verificado ✓' : 'Marcar verificado'}
                            </button>
